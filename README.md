...
- 高德结果缺评分或人均时，界面显示「评分待确认 / 人均待确认」，不会伪装成真实平台数据。
...
V3.3 让高德搜索结果在进入滑卡前先经过统一的质量处理层 `lib/restaurantQuality.ts`：

- 过滤无名称、缺少必要地点信息、明显非餐饮或距离明显过远的结果；不过度过滤，结果不足时自动补足。
- 按高德 `sourcePlaceId` 优先去重；缺少该 ID 时按标准化名称与地址去重，避免重复 POI，又不会轻易误删不同分店。
- 按距离、地址、评分、图片、菜系、预算、聚餐标签和真实 API 字段完整度评分排序。
- 菜系偏好先用于高德关键词搜索，再作为排序加分；严格菜系结果少时，会混入附近通用餐饮保证可滑数量。
- 预算只影响排序，不伪造高德未返回的人均。高德缺失价格显示「人均待确认（高德未提供）」，本地补足显示「体验版人均」。
- 每个房间目标生成 20 家候选；真实结果不足时先补同区域本地包、再补默认及通用本地包，最终仍写入同一个 `room_restaurants` 固定池。
- 无图或图片加载失败会按火锅、日料、烧烤、咖啡、面食、中餐、西餐、甜品等分类使用 `public/restaurants/` 本地图片兜底。
- 详情页明确区分「来自高德地点数据」「体验版餐厅数据」和「模拟食评，仅用于体验」，不会把 mock 数据写成真实平台评价。
...
- 继续完善高德 API 费用控制、调用限额、缓存刷新策略与真实数据合规。
